...
st.sidebar.image('https://encrypted-tbn0.gstatic.com/'
                 'images?q=tbn:ANd9GcS-Yjnkauyx5IIisCXFcP4CAMyYG41fm5cTgsWqeer6wY5I2M94k7sotloFTVneoGciMvkudlKsMwY&ec'
                 '=48600113')
...
if user_menu == 'Athlete wise Analysis':
    ...
    ax1 = sns.scatterplot(data=ath, x='Weight', y='Height', hue='Medal', style='Sex', s=200)
    st.pyplot(fig)
...
if user_menu == 'Country-wise Analysis':
    ...
    fig, ax = plt.subplots(figsize=(20, 20))
    ax2 = sns.heatmap(pt.astype('int'), annot=True)
    st.pyplot(fig)

    st.title("-------------------------------------------------------------")
    y_df = helper.top_athletes_countrywise(summer_olympics, selected_country)
    st.title('Top Athletes for ' + selected_country)
    z_df = y_df[['region', 'Name', 'Sport']].value_counts().reset_index().rename(
        columns={'count': 'Total Medals'}).head(10)
    st.table(z_df)
